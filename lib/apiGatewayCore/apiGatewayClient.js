...
apiGatewayCore = apiGatewayCore || {};

apiGatewayCore.apiGatewayClientFactory = {};
apiGatewayCore.apiGatewayClientFactory.newClient = function (simpleHttpClientConfig, sigV4ClientConfig) {
    var apiGatewayClient = { };
    //Spin up 2 httpClients, one for simple requests, one for SigV4
    var sigV4Client = apiGatewayCore.sigV4ClientFactory.newClient(sigV4ClientConfig);
    var simpleHttpClient = apiGatewayCore.simpleHttpClientFactory.newClient(simpleHttpClientConfig);
    ...
    apiGatewayClient.makeRequest = function (request, authType, additionalParams, apiKey) {
        ...
        request.headers = apiGatewayCore.utils.mergeInto(request.headers, additionalParams.headers);
        request.queryParams = apiGatewayCore.utils.mergeInto(request.queryParams, additionalParams.queryParams);
